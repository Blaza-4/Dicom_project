 ...
 * @param file pointer , group tag(using uint16_t because it should be 2 bytes),element tag,Value representation charecters,Actual string value)
*/
 ...
 * @param file pointer , group tag(using uint16_t because it should be 2 bytes),element tag,Value representation charecters,Actual number value(4 byte in size))
*/
 ...
 * @param file pointer , pixel array (using as 1 byte array),width and height both numbers);
*/
...
int main(int argc ,char * argv[]) {
    if(argc <= 1){
        printf("limited arguments");
        exit(-1);
    }
    const char *jpg_filename = argv[1];
    const char *dicom_filename = "out.dcm";
    ...
    write_dicom_tag(dicom, 0x0008, 0x0018, "UI", "1.3.6.1.4.1.14519.5.2.1.7695.2311.91678404998");   //sop instance UID
    ...
    write_dicom_tag(dicom, 0x0010, 0x0020,"LO", "C334");                          //patient ID
    ...
    write_dicom_tag(dicom,0x0020,0x000d,"UI","1.2.156.14702.1.1000.16.0.20200311113603875");      //study instance UID
    write_dicom_tag(dicom,0x020,0x00e,"UI","1.2.156.14702.1.1000.16.1.2020031111365295300020004");  //series instance UID
    
    write_dicom_tag(dicom,0x0020,0x0010,"SH","std101");
    write_dicom_tag(dicom,0x0020,0x0011,"IS","11"); 
    write_dicom_tag(dicom,0x0020,0x0013,"IS","1");
